...
import shap  # Импорт библиотеки SHAP для расчёта важности признаков
...
class Model:
    ...
    def train(self, data: pd.DataFrame):
        ...
        # Извлекаем важность признаков на основе базовой модели
        ...
        logger.info(f"Топ-10 признаков по важности (базовая модель):")
        ...
        # Сохраняем отчеты
        logger.info("\nВажность топ-10 признаков (базовая модель):")
        ...
        # ========= Отбор признаков на основе SHAP =========
        logger.info("Вычисление важности признаков с использованием SHAP...")
        explainer = shap.TreeExplainer(self.best_model)
        # Для ускорения расчёта используем подвыборку
        sample_size_shap = min(1000, X_train_advanced_scaled.shape[0])
        X_train_shap_sample = X_train_advanced_scaled[:sample_size_shap]
        shap_values = explainer.shap_values(X_train_shap_sample)
        # Среднее абсолютное значение SHAP для каждого признака
        mean_shap_values = np.mean(np.abs(shap_values), axis=0)
        
        # Формируем DataFrame с важностью признаков по SHAP
        shap_df = pd.DataFrame({
            'feature': advanced_feature_names,
            'shap_importance': mean_shap_values
        })
        shap_df.sort_values(by='shap_importance', ascending=False, inplace=True)
        logger.info("Топ-10 признаков по SHAP:")
        for i, row in shap_df.head(10).iterrows():
            logger.info(f"{row['feature']}: {row['shap_importance']:.4f}")
        
        # Отбор признаков по порогу — отбираем признаки, у которых важность выше 75-го перцентиля
        shap_threshold = np.percentile(mean_shap_values, 75)
        logger.info(f"Порог SHAP важности: {shap_threshold:.4f}")
        important_features = shap_df.loc[shap_df['shap_importance'] > shap_threshold, 'feature'].tolist()
        logger.info(f"Отобрано {len(important_features)} важных признаков из {len(advanced_feature_names)}")
        ...
        # Обучение модели на отобранных признаках (SHAP)
        logger.info("Обучение модели на отобранных признаках (с использованием SHAP)...")
        ...
        if metrics_important['rmse'] < best_overall_metrics['rmse']:
            logger.info("Модель с отобранными признаками (SHAP) показала лучший результат!")
            ...
            logger.info("Модель с отобранными признаками (SHAP) показала худший результат.")
        
        # Для отчёта создаём список кортежей (признак, SHAP важность)
        shap_feature_importance = list(zip(shap_df['feature'], shap_df['shap_importance']))
        save_metrics_report(metrics_important, shap_feature_importance, "Selected_Features")
        # ========= Конец отбора признаков на основе SHAP =========
