...
class Model:
    ...
    Теперь используется LightAutoML для автоматизированного обучения модели.
    """
    ...
    def train(self, data: pd.DataFrame):
        ...
        # Обучение модели LightAutoML
        logger.info("Обучение модели LightAutoML...")
        automl = TabularAutoML(task='reg', timeout=600, cpu_limit=-1)
        oof_preds = automl.fit_predict(train_data)
        
        logger.info("Выполнение предсказания на тестовой выборке LightAutoML...")
        ...
        logger.info("Сохранение модели LightAutoML...")
    ...
    def _load_model_state(self, model_path: str, features_path: str, top_features_path: str):
        ...
        logger.info("Модель LightAutoML успешно загружена.")

    def predict(self, data: pd.DataFrame):
        """
        Делает предсказание на основе лучшей модели LightAutoML.
        """
        logger.info("Выполнение предсказания LightAutoML...")
