...
2. Генерация признаков - сгенерированы признаки являющиеся результатом сложения, перемножения, статистическими функциями, экспонентами и тригонометрическими функциями от исходных признаков
...
- Boruta
  ```RMSE on validation set: 2.7543```
- Объединение
  ```RMSE on validation set: 2.2110```
#### Эксперименты с моделями

- RandomForest
  ```RMSE on validation set: 2.0532```
- LinearRegression
  ```RMSE on validation set: 2.7890```
- CatBoost
  ```RMSE on validation set: 2.1987```
- AutoML
  ```RMSE on validation set: 2.5678```
- AutoML Whitebox
  ```RMSE on validation set: 2.6112```
- AutoML с табличным трансформером
  ```RMSE on validation set: 2.4005```
- Ансамбль
  ```RMSE on validation set: 2.0123```
...
*   **Экономическая эффективность:** Оптимизация гранулометрического состава позволяет значительно снизить издержки. По нашим оценкам, экономия может достигать **30 миллионов рублей с каждого конвейера** ежегодно.
...
Для работы сервера мониторинга рекомендуются следующие минимальные характеристики:

*   **Оперативная память (RAM):** Не менее 8 ГБ свободной памяти.
*   **Процессор (CPU):** Система не требовательна к процессору. Тестирование проводилось на AMD Ryzen 9, что является избыточным; подойдет менее производительный современный процессор.
