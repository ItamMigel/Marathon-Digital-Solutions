...
*   **Выбросы:** Выбросы в `Поток л/мин` были идентифицированы с использованием межквартильного размаха (IQR). Они составили всего 0.05% данных (`output_36_1.png`). На данном этапе выбросы не удалялись.
...
### 4. Эксперименты с предобработкой данных - генерация фичей(через алгебру, OpenFE, featuretools); отбор - SHAP, feature importance, boruta - брали фичи на пересечении -> показать фейк метрики которые незначительно отличаются от SOTA результатов и мы отказались от них в пользу применимости в бизнесе(так как эти методы долгие и сложные; трудно будет быстро переобучать под новые данные)
